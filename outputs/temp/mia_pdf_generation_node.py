...
class NumberedCanvas(canvas.Canvas):
    ...
    def _draw_decorations(self, total_pages: int):
        ...
        # ── Diagonal "AI-ASSISTED ANALYSIS" watermark ──
        self.saveState()
        self.setFont("Helvetica-Bold", 48)
        self.setFillColorRGB(0.85, 0.85, 0.85, alpha=0.12)
        self.translate(A4[0] / 2, A4[1] / 2)
        self.rotate(35)
        self.drawCentredString(0, 0, "AI-ASSISTED ANALYSIS")
        self.restoreState()
...
def get_styles():
    ...
    defs = {
        ...
        "SecHead":    dict(fontSize=13, textColor=C["white"], spaceAfter=6,
...
def _section_header(title: str, color=None) -> Table:
    ...
    # Left colored bar + title in one row
    bar  = Table([[""]],  colWidths=[6], rowHeights=[28])
    bar.setStyle(TableStyle([("BACKGROUND", (0,0), (-1,-1), color)]))
    text = Paragraph(f"<b>{title}</b>", styles["SecHead"])
    row  = Table([[bar, text]], colWidths=[10, 440])
...
def _cover_page(report: MIAReport) -> list:
    ...
    banner_data = [[Paragraph(
        f"<b>{urg['emoji']}  URGENCY: {urgency_val}</b>",
...
def _findings_summary(report: MIAReport) -> list:
    ...
    header_row = [
        Paragraph("<b>ID</b>",   styles["Label"]),
        Paragraph("<b>Location</b>", styles["Label"]),
        Paragraph("<b>Description</b>", styles["Label"]),
        Paragraph("<b>Severity</b>", styles["Label"]),
        Paragraph("<b>Measurements (mm)</b>", styles["Label"]),
        Paragraph("<b>Clinical Significance</b>", styles["Label"]),
    ]
    ...
    stats_data = [
        [Paragraph("🚨 Critical/Severe", styles["Label"]), str(crit),
         Paragraph("⚠️ Moderate",         styles["Label"]), str(mod),
...
def _medical_image_page(report: MIAReport) -> list:
    ...
    story.append(Spacer(1, 20))
    story.append(_section_header(f"🖼️  {image_type.upper()} IMAGE", color=C["primary"]))
    story.append(Spacer(1, 12))
...
def _safety_page(report: MIAReport) -> list:
    ...
    styles = get_styles()
    story.append(Spacer(1, 20))
    story.append(_section_header("🛡️  SAFETY ANALYSIS", color=C["critical"]))
    story.append(Spacer(1, 10))
...
def _technical_page(report: MIAReport) -> list:
    ...
    styles = get_styles()
    story.append(Spacer(1, 20))
    story.append(_section_header("⚙️  TECHNICAL DETAILS & QUALITY ASSURANCE"))
    story.append(Spacer(1, 10))
    ...
    story.append(Spacer(1, 16))
    story.append(_section_header("⚠️  DISCLAIMER", color=C["critical"]))
    story.append(Spacer(1, 8))
    story.append(Paragraph(
...
class PDFGenerationNode:
    ...
    def create_impression(self, report_data: Dict[str, Any], 
        ...
        if report_data:
            ...
            if report_text:
                impression_parts.append(report_text[:1000])  # Limit to 1000 chars
        
        # Add clinical correlation
        if report_data and "clinical_correlation" in report_data:
            impression_parts.append(f"\n\nClinical Correlation: {report_data['clinical_correlation']}")
        
        # Add safety notes if critical
        if safety_data:
